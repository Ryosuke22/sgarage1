...
export default function CreateListing() {
  ...
  const getModelsForYearAndMaker = (year: number, maker: string, category: string): string[] => {
    const categoryData = historicalModels[category as keyof typeof historicalModels];
    if (!categoryData || !categoryData[maker]) {
      console.log("No data for category/maker:", category, maker);
      return [];
    }
    
    const models = categoryData[maker];
    const availableModels = models.filter(model => {
      const isAvailable = year >= model.startYear && (!model.endYear || year <= model.endYear);
      if (!isAvailable) {
        console.log(`Filtering out ${model.name} for ${year} (available: ${model.startYear}-${model.endYear || 'present'})`);
      }
      return isAvailable;
    }).map(model => model.name);
    
    console.log(`Models for ${maker} ${year} (${category}):`, availableModels);
    return availableModels.sort();
  };

  const handleMakerChange = (maker: string) => {
    setSelectedMaker(maker);
    console.log("Maker selected:", maker);
    // Reset year and model when maker changes
    form.setValue("make", maker);
  ...
  const handleYearChange = (year: number) => {
    ...
    console.log("Year selected:", year, "Maker:", maker, "Category:", category);
    
    form.setValue("year", year);
    form.setValue("model", ""); // Reset model when year changes
    
    if (maker && category && year) {
      console.log("Updating models for:", { maker, year, category });
      const models = getModelsForYearAndMaker(year, maker, category);
      console.log("Available models:", models);
      setAvailableModels(models);
    } else {
      console.log("Missing requirements:", { maker: !!maker, category: !!category, year: !!year });
    }
    setIsCustomModel(false);
  };
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <Select onValueChange={handleMakerChange} defaultValue={field.value}>
                          <FormControl>
                            <SelectTrigger data-testid="select-make" className={form.formState.errors.make ? "border-red-500" : ""}>
                              <SelectValue placeholder={
                                form.watch("category") ? "メーカーを選択" : "カテゴリを先に選択してください"
                              } />
                            </SelectTrigger>
                          </FormControl>
                          <SelectContent className="max-h-[200px]">
                            {form.watch("category") ? (
                              form.watch("category") === "car" ? (
                                Object.keys(carMakersAndModels).map((maker) => (
                                  <SelectItem key={maker} value={maker}>
                                    {maker}
                                  </SelectItem>
                                ))
                              ) : (
                                Object.keys(motorcycleMakersAndModels).map((maker) => (
                                  <SelectItem key={maker} value={maker}>
                                    {maker}
                                  </SelectItem>
                                ))
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        >
                          ...
                          <SelectContent className="max-h-[200px]">
                            {selectedMaker ? (
                              Array.from({ length: 2001 - 1950 + 1 }, (_, i) => 2001 - i).map((year) => (
                                <SelectItem key={year} value={year.toString()}>
                                  {year}年
                                </SelectItem>
                              ))
                            ) : (
                              <SelectItem value="none" disabled>Please select manufacturer first</SelectItem>
                            )}
                          </SelectContent>
                        </Select>
                        <FormMessage className="text-red-500 text-xs mt-1" />
                      ...
                      <FormItem>
                        ...
                        {isCustomModel ? (
                          ...
                          <Select onValueChange={handleModelChange} defaultValue={field.value}>
                            ...
                            <SelectContent className="max-h-[200px]">
                              {(() => {
                                console.log("Rendering models in dropdown:", { 
                                ...
                                if (availableModels.length > 0) {
                                  ...
                                  return (
                                    <SelectItem value="none" disabled>
                                      {!selectedMaker ? "メーカーを先に選択してください" : "年式を先に選択してください"}
                                    </SelectItem>
                                  );
                                }
                              })()}
            ...
            <Card className="bg-gray-800/50 border-white/10">
              ...
              <CardContent className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {/* 車検証 */}
                  <div className="space-y-2">
                    <label className="text-sm font-medium">車検証</label>
                    <div className="flex flex-col items-center justify-center gap-2 py-6 border border-gray-200 rounded-lg bg-gray-50 text-gray-500">
                      <span className="text-2xl">📋</span>
                      <span className="text-sm font-medium">書類アップロード（準備中）</span>
                      <span className="text-xs">現在、画像のアップロード機能のみ利用可能です</span>
                    </div>
                  </div>

                  {/* 譲渡証明書 */}
                  <div className="space-y-2">
                    <label className="text-sm font-medium">譲渡証明書</label>
                    <div className="flex flex-col items-center justify-center gap-2 py-6 border border-gray-200 rounded-lg bg-gray-50 text-gray-500">
                      <span className="text-2xl">📄</span>
                      <span className="text-sm font-medium">書類アップロード（準備中）</span>
                      <span className="text-xs">現在、画像のアップロード機能のみ利用可能です</span>
                  ...
                  {/* 印鑑証明書 */}
                  <div className="space-y-2">
                    <label className="text-sm font-medium">印鑑証明書</label>
                    <div className="flex flex-col items-center justify-center gap-2 py-6 border border-gray-200 rounded-lg bg-gray-50 text-gray-500">
                      <span className="text-2xl">🔖</span>
                      <span className="text-sm font-medium">書類アップロード（準備中）</span>
                      <span className="text-xs">現在、画像のアップロード機能のみ利用可能です</span>
                  ...
                  {/* 自賠責保険証明書 */}
                  <div className="space-y-2">
                    <label className="text-sm font-medium">自賠責保険証明書</label>
                    <div className="flex flex-col items-center justify-center gap-2 py-6 border border-gray-200 rounded-lg bg-gray-50 text-gray-500">
                  ...
                  {/* 整備記録簿 */}
                  <div className="space-y-2">
                    <label className="text-sm font-medium">整備記録簿</label>
                    <div className="flex flex-col items-center justify-center gap-2 py-6 border border-gray-200 rounded-lg bg-gray-50 text-gray-500">
                      <span className="text-2xl">🔧</span>
                      <span className="text-sm font-medium">書類アップロード（準備中）</span>
                      <span className="text-xs">現在、画像のアップロード機能のみ利用可能です</span>
                  ...
                  {/* その他書類 */}
                  <div className="space-y-2">
                    <label className="text-sm font-medium">その他書類</label>
                    <div className="flex flex-col items-center justify-center gap-2 py-6 border border-gray-200 rounded-lg bg-gray-50 text-gray-500">
                      <span className="text-2xl">📎</span>
                      <span className="text-sm font-medium">書類アップロード（準備中）</span>
                      <span className="text-xs">現在、画像のアップロード機能のみ利用可能です</span>
