      ...
      <div className="mx-auto max-w-6xl p-4 space-y-6">
        ...
        <Section title="詳細情報">
          <div className="space-y-2">
            <Row k="事故歴" v={<Fallback v={d.hasAccidentHistory === "yes" ? "あり" : d.hasAccidentHistory === "no" ? "なし" : "不明"} />} />
            <Row k="購入年" v={<Fallback v={d.purchaseYear} />} />
            <Row k="改造パーツ" v={<pre className="whitespace-pre-wrap text-sm"><Fallback v={d.modifiedParts} /></pre>} />
            <Row k="購入前情報" v={<pre className="whitespace-pre-wrap text-sm"><Fallback v={d.prePurchaseInfo} /></pre>} />
            <Row k="所有者メンテナンス" v={<pre className="whitespace-pre-wrap text-sm"><Fallback v={d.ownerMaintenance} /></pre>} />
          </div>
        </Section>
        ...
        <section className="rounded-lg border p-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm">
            ...
            <Row k="開始価格" v={<>{d.startingPrice ? `¥${Number(d.startingPrice).toLocaleString()}` : "—"}</>} />
            <Row k="リザーブ価格" v={<>{d.reservePrice ? `¥${Number(d.reservePrice).toLocaleString()}` : "—"}</>} />
